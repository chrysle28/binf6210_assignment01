...
df_Araneae <- data_BOLD_Araneae %>% 
  filter(!is.na(elev)) ### Removing records with N/A elevation values as they cannot contribute to analysis
#==> Number of records went from 205445 to 127907
any(is.na(df_Araneae$elev)) #PC
...
#==> Number of records for went from 127907 to 58207
...
#*** Account for sampling effort using two methods: 1) including sampling effort as a covariate in regression model 2) rarefying richness
...
df_Aves <- data_BOLD_Aves %>% 
  filter(!is.na(elev)) ### Removing records with N/A elevation values as they cannot contribute to analysis
#==> Number of records for Aves went from 67047 to 9570
any(is.na(df_Aves$elev)) #PC
